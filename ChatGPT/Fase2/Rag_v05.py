    ...
    if modelName == "GPT":
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": ("Provide me the mapping between the columns of the table and the corresponding FHIR attributes." )},
            ],
            temperature=0,
        )
        ...
        # Bucle para iteraciones de reflexión (si es necesario)
        for i in range(iterations):
            reflection_prompt = client.chat.completions.create(
                model="meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo",
                messages=[
                    {"role": "system", "content": ( f"Reflect on this mapping: {response_content}. Identify any inconsistencies, possible improvements, or errors, and provide a corrected version.")},
                    {"role": "user", "content": ("Please review mapping and provide a corrected version. If possible.")},
                ],
                temperature=0,
            )

            # Procesar la respuesta de reflexión
            response_content = reflection_prompt.choices[0].message.content
...
for filename in filenames[start_index:]:
    ...
    with open("C:/Users/Alvaro/ChatGPT/ChatGPT/Fase3/mapeos/mapeosReflexiveSerial_NoSchema/mapeo5_"+filename+".txt", 'a') as archivo:
        ...
        response = generate_response(query, "Llama", context, 5)
