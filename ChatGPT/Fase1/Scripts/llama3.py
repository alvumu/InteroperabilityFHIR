        ...
        for line in lines:
          if "Patient Info:" in line:
            ...
            if prompt_type == "few_shot_basic" :      
                ...
                ##Output: You must only answer generate the FHIR resource about the serialized data as Plain Text format simulating a JSON. Do not make any comments.
                     ...
                     prompt = f'''I will provide you a few examples of serialized data and the corresponding FHIR resource. Your task is to generate a FHIR resource based on the information about the patient .     
                                  ...
                                  - Objective Data:
                                      - Patient to Generate: {patient_to_generate}
                                                        ...
                                                        - When the patient race extension is "Black or African American" set the the code "2054-5".                                                          
                                                        - When the patient ethnicity extension is "Hispanic or Latino" set the code to "2135-2"
                                                        - When the patient race is "American Indian or Alaska Native" se the code to "1002-5"
                                                        ##Output: 
                                                        -You must only generate the FHIR resource as JSON format following the example resources. Avoid to generate any additional information. Do not generate any code or comments.
...
"""
            ...
            print(contador)
            # Acceder al campo identifier y luego al campo value
            expresion_identifier = r'(?<="value": ")\d{8}(?=")'
            ...
            if prompt_type == "few_shot_basic" :      
                ...
                """
                     ...
                     prompt = f'''I will provide you a few examples of serialized data and the corresponding FHIR resource. Your task is to generate a FHIR resource based on the information about the patient .     
                                  ...
                                  ##Mapping instructions:
                                    - When the patient race is "American Indian or Alaska Native" se the code to "1002-5"
                                    - When the patient ethnicity is "Hispanic or Latino" set the code to "2135-2"
                                    - When the patient race is "Black or African American" set the the code "2054-5".                                                          
...
"""
            ...
            expresion_regular = r'\{(?:[^{}]|(?R))*\}'
            print(fhir_generated)
            fhir_generated = re.findall(expresion_regular, fhir_generated)[1]
            print(fhir_generated)
            response = requests.post(url, fhir_generated, params={"profile": profile} )
            validationAPI = response.json()['issue'][0]['severity']
            fhir_generated_dict = json.loads(fhir_generated) 
...
#generate_and_validate_fhir_resource("iterative_prompt", profile, "JSON", "SerializedData/serialized_good_json-preprocessed.json", "Llama3/Patient/Good/resultados_iterative_prompt_good_json_pre_2.ndjson")
generate_and_validate_fhir_resource("iterative_prompt", profile, "Plain Text", "SerializedData/serialized_good_txt-preprocessed.json", "Llama3/Patient/Good/resultados_iterative_prompt_good_txt_pre_2.ndjson")
